  ...
  return (
    <div ref={containerRef} className={`flex flex-col items-center relative mt-4 w-1/2 ${!last && 'min-h-[300px]'}`}>
      ...
      <motion.div 
        ...
        className={`absolute flex flex-col ${side === 'left' ? 'left-16 items-start text-start' : 'right-16 items-end text-end'}`}
      >
        <h1 className={`text-white text-3xl w-100`}>{name}</h1>
        <h2 className={`text-white/50 text-2xl`}>{title}</h2>
        <span className={`text-xl font-bold flex flex-row ${side === 'left' ? 'justify-start' : 'justify-end' } w-30`}>{period}</span>
      </motion.div>
